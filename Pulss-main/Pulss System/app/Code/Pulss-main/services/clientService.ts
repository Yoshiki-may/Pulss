...
const FRONT_BASE =
  import.meta.env.VITE_FRONT_BASE_URL || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:5173');
...
export const clientService = {
  ...
  async generatePulseUrl(clientId: string): Promise<string> {
    // Legacy fallback (kept for compatibility with existing button)
    try {
  ...
  async generatePulssLink(clientId: string): Promise<{ url: string; token?: string }> {
    try {
      const data = await request(`/api/clients/${clientId}/pulss-link`, { method: 'POST' });
      return { url: data.url, token: data.token };
    } catch (e) {
      handleError(e);
      // Fallbackは旧pulse.example.comではなく現行のpulss-systemパスを生成
      const token = Math.random().toString(36).slice(2, 10);
      const url = `${FRONT_BASE}/pulss-system/${token}`;
      return { url, token };
    }
  },
