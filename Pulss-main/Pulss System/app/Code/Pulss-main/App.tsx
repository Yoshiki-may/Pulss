...
import SalesBoardPage from './components/Sales/SalesBoardPage';
import HearingPage from './components/Hearings/HearingPage';
import ProposalBoardPage from './components/Proposals/ProposalBoardPage';
import ContractsPage from './components/Contracts/ContractsPage';
import TasksBoardPage from './components/Tasks/TasksBoardPage';
import ContentCalendarPage from './components/Calendar/ContentCalendarPage';
import ReportsPage from './components/Reports/ReportsPage';
import NotificationsPage from './components/Notifications/NotificationsPage';
import PulssSystemChatPage from './components/PulssSystem/PulssSystemChatPage';
...
const App: React.FC = () => {
  ...
  if (pathname.startsWith('/pulss-system')) {
    const parts = pathname.split('/').filter(Boolean); // ["pulss-system", clientId, token]
    const clientId = parts[1] || '';
    const token = parts[2] || '';
    return <PulssSystemChatPage clientId={clientId} token={token} />;
  }
  ...
  return (
    <div className="flex h-screen w-full bg-white overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-full min-w-0 bg-white">
        ...
        {currentView === 'sales' && <SalesBoardPage />}
        {currentView === 'hearings' && <HearingPage />}
        {currentView === 'proposals' && <ProposalBoardPage />}
        {currentView === 'contracts' && <ContractsPage />}
        {currentView === 'tasks' && <TasksBoardPage />}
        {currentView === 'calendar' && <ContentCalendarPage />}
        {currentView === 'reports' && <ReportsPage />}
        {currentView === 'notifications' && <NotificationsPage />}
        {currentView === 'news' && <SnsNewsPage />}
        {currentView === 'director' && <DirectorBoardPage onSelectClient={handleSelectClient} />}
