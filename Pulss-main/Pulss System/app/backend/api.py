...
class PulssLinkOut(BaseModel):
    url: str
    token: str


class PulssChatStartOut(BaseModel):
    session_id: str
    client_name: Optional[str]
    first_message: str


class PulssChatMessagePayload(BaseModel):
    user_message: str


class PulssChatMessageOut(BaseModel):
...
class AiDraftOut(BaseModel):
    ...
    @classmethod
    def from_domain(cls, draft: AiDraft) -> "AiDraftOut":
...
def build_router(
    ...
    def create_pulse_link(client_id: str) -> dict:
        ...
        url = f"{pulss_service.front_base_url}/pulss-system/{client_id}/{link.token}"
        return {"url": url, "token": link.token}

    @router.post("/clients/{client_id}/pulss-link", response_model=PulssLinkOut)
    def create_pulss_link(client_id: str) -> PulssLinkOut:
        client = client_service.get_client(client_id)
        if not client:
            raise HTTPException(status_code=404, detail="Client not found")
        link = pulss_service.issue_link(client_id)
        url = f"{pulss_service.front_base_url}/pulss-system/{client_id}/{link.token}"
        return PulssLinkOut(url=url, token=link.token)
    ...
    @router.post("/pulss-chat/start-from-link/{client_id}/{token}", response_model=PulssChatStartOut)
    def start_pulss_chat_with_client(client_id: str, token: str) -> PulssChatStartOut:
        logger.info("[pulss] start-from-link (with client) called: client_id=%s token=%s", client_id, token)
        try:
            started = pulss_service.start_session_from_client_token(client_id, token)
        except PulssLinkNotFound as e:
            logger.info("[pulss] start-from-link not found/expired/mismatch: client_id=%s token=%s", client_id, token)
            raise HTTPException(status_code=404, detail=str(e))
        except PulssOpenAIError:
            logger.exception("[pulss] start-from-link openai error: client_id=%s token=%s", client_id, token)
            raise HTTPException(status_code=502, detail="Failed to call OpenAI API")
        except PulssPersistenceError:
            logger.exception("[pulss] start-from-link persistence error: client_id=%s token=%s", client_id, token)
            raise HTTPException(status_code=500, detail="Failed to create session")
        except Exception:  # noqa: BLE001
            logger.exception("[pulss] unexpected error in start-from-link: client_id=%s token=%s", client_id, token)
            raise HTTPException(status_code=500, detail="Internal server error")

        if not started:
            logger.info("[pulss] start-from-link returned None (treated as not found): client_id=%s token=%s", client_id, token)
            raise HTTPException(status_code=404, detail="Link not found or expired")
        session, first_message, client = started
        logger.info("[pulss] start-from-link success: session_id=%s client_id=%s", session.id, session.client_id)
        return PulssChatStartOut(
            session_id=session.id,
            client_name=client.name if client else None,
            first_message=first_message,
        )

    @router.post("/pulss-chat/start-from-link/{token}", response_model=PulssChatStartOut)
    def start_pulss_chat(token: str) -> PulssChatStartOut:
        logger.info("[pulss] start-from-link called: token=%s", token)
        try:
            started = pulss_service.start_session_from_token(token)
        except PulssLinkNotFound as e:
            logger.info("[pulss] start-from-link not found/expired: token=%s", token)
            raise HTTPException(status_code=404, detail=str(e))
        except PulssOpenAIError as e:
            logger.exception("[pulss] start-from-link openai error: token=%s", token)
            raise HTTPException(status_code=502, detail="Failed to call OpenAI API")
        except PulssPersistenceError as e:
            logger.exception("[pulss] start-from-link persistence error: token=%s", token)
            raise HTTPException(status_code=500, detail="Failed to create session")
        except Exception as e:  # noqa: BLE001
            logger.exception("[pulss] unexpected error in start-from-link: token=%s", token)
            raise HTTPException(status_code=500, detail="Internal server error")

        if not started:
            logger.info("[pulss] start-from-link returned None (treated as not found): token=%s", token)
            raise HTTPException(status_code=404, detail="Link not found or expired")
        session, first_message, client = started
        logger.info("[pulss] start-from-link success: session_id=%s client_id=%s", session.id, session.client_id)
        return PulssChatStartOut(
    ...
    @router.post("/pulss-chat/sessions/{session_id}/messages", response_model=PulssChatMessageOut)
    def send_pulss_message(session_id: str, payload: PulssChatMessagePayload) -> PulssChatMessageOut:
        result = pulss_service.post_message(session_id, payload.user_message)
        if not result:
            raise HTTPException(status_code=404, detail="Session not found")
        assistant_message, done = result
        return PulssChatMessageOut(assistant_message=assistant_message, done=done)
    ...
    @router.post("/clients/{client_id}/ai-drafts", response_model=AiDraftOut)
    def create_ai_draft(client_id: str, payload: AiDraftPayload) -> AiDraftOut:
        client = client_service.get_client(client_id)
        if not client:
            raise HTTPException(status_code=404, detail="Client not found")
        draft = pulss_service.save_ai_draft(client_id, payload.type, payload.status, payload.content)
        return AiDraftOut.from_domain(draft)
